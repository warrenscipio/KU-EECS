...
template <typename T>
void mergeInsertion(T theArray[], long first, long last, T* tempArray, long threshold)
{

   if((last-first+1)<= threshold){//if less than or equal to threshold do insertion
	insertionSort(theArray, (last-first+1));
   }else if (first < last)
   {  // sort each half
      long mid = (first + last)/2;    // index of midpoint
      // sort left half theArray[first..mid]
      mergeInsertion(theArray, first, mid, tempArray,threshold);
      // sort right half theArray[mid+1..last]
      mergeInsertion(theArray, mid+1, last, tempArray,threshold);
...
template <typename T>
void mergeBubble(T theArray[], long first, long last, T* tempArray, long threshold)
{

   if((last-first+1)<= threshold){// if less than or equal to threshold do bubble
	bubbleSort(theArray, (last-first+1));
   }else if (first < last){  // sort each half
      long mid = (first + last)/2;    // index of midpoint
      // sort left half theArray[first..mid]
      mergeBubble(theArray, first, mid, tempArray, threshold);
      // sort right half theArray[mid+1..last]
      mergeBubble(theArray, mid+1, last, tempArray, threshold);
...
template <typename T>
void mergesort(T theArray[], long size, long thresh, int n)// created another para to differintiate different versions of merge sort
{

	T* tempArray = new T[size];
	//decides which version of merge sort to do
	if(n==0)
		mergeInsertion(theArray, 0, size-1, tempArray, thresh);
	else if( n==1)
		mergeBubble(theArray, 0, size-1, tempArray, thresh);
	else
