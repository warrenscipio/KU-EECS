...
//read meathod
template <typename T>
void Executive::read(istream& is, CountArray<T>& arr, string name){
	...
	while(is >> t){
		count++;
		// while reading call an instance of CountArray to use the method bump count
		arr.bumpCount(t);
		//call method to print out every 5

		if (count%5 ==0){
		times++;
		cout << "Printing out " << count << " items " << "from the " << name << endl;;
		print(arr);
		}
...
template <typename T>
void Executive::print(CountArray<T> arr){ // arr MUST be passed by value
