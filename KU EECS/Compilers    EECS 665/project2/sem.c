...
void dobreak()
{
	fprintf(stderr, "never test this\n");
	startscope->topMost = merge(
...
void dodo(int m1, int m2, struct sem_rec *e, int m3)
{
	...
	backpatch(startscope->topMost, m2);
...
void dogoto(char *id)
{
   fprintf(stderr, "sem: dogoto not implemented YET\n");
}
...
{
	backpatch(startscope->last, m);

	startscope = scopes[--depth - 1];
}
...
void startloopscope()
{
	...
	startscope = scopes[depth++];

	startscope->topMost = NULL;
	startscope->last = NULL;
}
