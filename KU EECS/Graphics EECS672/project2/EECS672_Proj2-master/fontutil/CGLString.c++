...
CGLString::CGLString(const char *text, const CFont *font, int dim) :
	...
	vao(0), vboVertexCoords(0), vboTexCoords(0)
{
...
void CGLString::render(int pvaLoc_wcPos, int pvaLoc_texCoords, int ppuLoc_texMap)
{
	if (renderingParametersModified)
		sendBufferDataToGPU(pvaLoc_wcPos, pvaLoc_texCoords);
	...
	glDrawElements(GL_TRIANGLES, numIndices, GL_UNSIGNED_BYTE, mIndices);
...
void CGLString::sendBufferDataToGPU(int pvaLoc_wcPos, int pvaLoc_texCoords)
{
	...
	if (vao == 0) // First call; allocate and fill all structures
	{
		...
		glEnableVertexAttribArray(pvaLoc_wcPos);
		glVertexAttribPointer(pvaLoc_wcPos, vertexCoordsDimension, GL_FLOAT, GL_FALSE, 0, 0);
